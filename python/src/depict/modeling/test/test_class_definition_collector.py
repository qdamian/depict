...
class TestClassDefinitionCollector(unittest.TestCase):
    ...
    def test_registers_itself_in_source_code_parser(self):
        ...
        class_def_locator = ClassDefinitionCollector(source_code_parser_mock, Mock())
        source_code_parser_mock.register.assert_called_once_with(class_def_locator)

    def test_adds_one_class_to_repo(self):
        with patch('depict.modeling.class_definition_collector.global_module_repo') as module_repo_mock:
            with patch('depict.modeling.class_definition_collector.global_class_repo') as class_repo_mock:
                entity_id_generator_mock = Mock()
                class_def_locator = ClassDefinitionCollector(Mock(), entity_id_generator_mock)
                fake_node = Mock()
    ...
    def test_finds_module_to_init_the_class(self):
        with patch('depict.modeling.class_definition_collector.global_class_repo') as class_repo_mock:
            with patch('depict.modeling.class_definition_collector.global_module_repo') as module_repo_mock:
                ...
                class_def_locator = ClassDefinitionCollector(source_code_parser_mock, entity_id_generator_mock)
