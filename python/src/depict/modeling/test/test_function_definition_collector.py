...
class TestFunctionDefinitionCollector(unittest.TestCase):
    ...
    def test_register_itself_with_source_code_parser(self):
        ...
        class_def_locator = FunctionDefinitionCollector(source_code_parser_mock, Mock())
    ...
    def test_one_function_is_added_to_function_repo(self):
        with patch('depict.modeling.function_definition_collector.global_function_repo') as function_repo_mock:
            source_code_parser_mock = Mock()
            entity_id_generator_mock = Mock()
            entity_id_generator_mock.create.return_value = 'fake_file_name.py:44'
            function_def_collector = FunctionDefinitionCollector(source_code_parser_mock,
    ...
    @patch('depict.modeling.function_definition_collector.global_class_repo')
    @patch('depict.modeling.function_definition_collector.global_function_repo')
    def test_one_method_is_added_to_function_repo(self, function_repo_mock, class_repo_mock):
                ...
                function_definition_collector = FunctionDefinitionCollector(Mock(), entity_id_generator)
                fake_node = Mock()
